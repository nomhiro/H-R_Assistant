...
class CosmosService:
    def __init__(self):
        self.client = CosmosClient.CosmosClient(
            COSMOSDB_URI, {'masterKey': COSMOSDB_KEY})
    ...
    def insert_data(self, data):
        ...
        logging.info(f"Inserted data into CosmosDB")

    def get_data(self, query):
        items = list(self.container.query_items(
            query=query, enable_cross_partition_query=True))
        logging.info(f"Got data from CosmosDB")
        return items
    ...
    def update_data(self, query, data):
        ...
        logging.info(f"Updated data in CosmosDB")
        return items
